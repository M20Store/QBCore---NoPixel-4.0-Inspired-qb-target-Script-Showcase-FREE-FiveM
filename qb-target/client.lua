...
local listSprite = {}
local interaction3dList = {}
local interaction3dList_near = {}
...
local function RaycastCamera(flag, playerCoords)
    ...
    if not DoesEntityExist(playerPed) then
        print("Error: PlayerPed is invalid.")
        return
    end
    ...
    if not rayPos or not rayDir then
        print("Error: Invalid ray position or direction.")
        return
    end

    local destination = rayPos + 16 * rayDir
    local success, rayHandle = pcall(StartShapeTestRay, rayPos.x, rayPos.y, rayPos.z, destination.x, destination.y, destination.z, flag or -1, playerPed, 4)
    
    if not success then
        print("Error starting shape test.")
        return
    end
    
    -- print("Ray Position: ", rayPos, "Ray Direction: ", rayDir)
    ...
    while true do
        ...
        if result ~= 1 then
            ...
            if flag == 30 and entityHit then
                entityHit = HasEntityClearLosToEntity(entityHit, playerPed, 7) and entityHit or nil
            end

            local entityType = entityHit and GetEntityType(entityHit) or 0

            if entityType == 0 and entityHit and pcall(GetEntityModel, entityHit) then
                entityType = 3
            end
...
local function DrawTarget()
	CreateThread(function()
		while not HasStreamedTextureDictLoaded("dcelements") do Wait(10) RequestStreamedTextureDict("dcelements", true) end
		...
		while targetActive do
			...
			for _, zone in pairs(listSprite) do
				...
				DrawSprite("dcelements", "point", 0, 0, 0.012, 0.025, 0, r, g, b, a)
				-- print(alpha)
				DrawSprite('dcelements', 'point', 0, 0, 0.012, 0.025, 0, r, g, b, alpha)
				DrawSprite('dcelements', 'point', 0, 0, 0.012, 0.025, 0, 10, 228, 170, alpha)
				ClearDrawOrigin()
			end
			Wait(sleep)
		end
					...
					if next(nuiData) then
						...
						while targetActive and success do
							...
							if not closestZone:isPointInside(newCoords) or dist > closestZone.targetoptions.distance then
								...
								DrawOutlineEntity(entity, false)
								break
							elseif not hasFocus and not closestZone.targetoptions.context and IsDisabledControlPressed(0, Config.MenuControlKey) then
								EnableNUI(nuiData)
								DrawOutlineEntity(entity, false)
							elseif not hasFocus and closestZone.targetoptions.context and IsDisabledControlPressed(0, Config.MenuControlKey2) then
								-- print(111)
								EnableNUI(nuiData)
								DrawOutlineEntity(entity, false)
							end
...
local function SetOptions(tbl, distance, options)
	for _, v in pairs(options) do
		if type(v) == "table" and v.required_item ~= nil then
