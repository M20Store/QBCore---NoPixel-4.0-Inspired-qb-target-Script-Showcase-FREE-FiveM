...
Config.SuccessDrawColor = {229, 99, 12, 255}
...
Config.OutlineColor = {229, 99, 12, 255}
...
local function CompanyCheck() return true end
...
CreateThread(function()
	...
	if Config.Standalone then
		...
		CompanyCheck = function(company)
			if type(company) == 'table' then
				company = company[PlayerData.company.name]
				if company and PlayerData.company.grade.level >= company then
					return true
				end
			elseif company == 'all' or company == PlayerData.company.name then
				return true
			end
			return false
		end
		...
		RegisterNetEvent('QBCore:Client:OnCompanyUpdate', function(ComInfo)
			PlayerData.company = ComInfo
		end)
...
function CheckOptions(data, entity, distance)
	...
	if data.company and not CompanyCheck(data.company) then return false end
